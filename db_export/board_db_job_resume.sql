...
CREATE TABLE `job_resume` (
  `jrs_id` int NOT NULL,
  `jrs_type` varchar(255) DEFAULT NULL,
  `jrs_text` text,
  `jrs_created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `jc_code` varchar(16) NOT NULL,
  PRIMARY KEY (`jrs_id`),
  KEY `fk_job_resume_category` (`jc_code`),
  CONSTRAINT `fk_job_resume_category` FOREIGN KEY (`jc_code`) REFERENCES `job_categories` (`jc_code`) ON DELETE RESTRICT ON UPDATE CASCADE
